...
try {
    ...
    let user;
    user = await studentSchema.findOne({ email }); // First try student collection
    ...
    res.cookie('jwt', token, { httpOnly: true, secure: true, sameSite: 'strict' }); // Adjust flags as needed
} catch (err) {
    console.error(err);
